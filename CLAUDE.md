...
> **START HERE →** Session 15 is done — **notifications, Phase 3's third slice.** New `Notification` model (`{recipientId, boardId, itemId, updateId, type, readAt?, emailSentAt?}`, `@@unique([recipientId, updateId])` — the literal "deduped" mechanism Phase 3's done-when names). **In-app notifications are fully real and automatic**: `createUpdate` writes one `Notification` row per mentioned recipient in the *same transaction* as the comment (self-mentions excluded), the same reasoning `ActivityLog` already uses — first-party application data, not a cross-boundary delivery problem, so no queue needed. **Email is the one piece that genuinely needed outbox/relay treatment** (§7.1): `server/services/notificationRelay.ts`'s `deliverMentionEmails()` reads unpublished `OutboxEvent` rows (now carrying `mentionedUserIds` in their payload) and sends via a new `lib/mailer.ts` (nodemailer, reusing the existing `EMAIL_SERVER`/`EMAIL_FROM` NextAuth already uses for magic links) — idempotent via `emailSentAt`/`publishedAt` checks, so calling it twice never double-sends. **Per your answer, `deliverMentionEmails()` is deliberately left unscheduled this session** — same precedent as `rebalanceRanks` (Session 3): built, fully tested, correct, but nothing calls it automatically. Flagged honestly: Phase 3's done-when isn't *fully* met until something (Phase 4's worker, or an interim cron) actually invokes it on a schedule — the seed script calls it once so `pnpm db:seed` proves the path end-to-end, but that's a one-time proof, not continuous delivery. `notification.*` procedures are session-scoped (`recipientId === ctx.userId`), not board-scoped — the first router in this codebase with no `requireBoardAccess` gate. `docker-compose.yml` gained a `maildev` service so sent email is actually visible locally (`localhost:1080`). Next action: Phase 3's remaining named piece is file attachments (S3 + local-disk dev adapter) — no session plan exists for it yet in §10.1, write one before starting. Separately worth remembering: the unscheduled-relay gap above is real and should be closed before Phase 3 is truly "done," likely alongside whatever Phase 4 session first stands up the BullMQ worker.
