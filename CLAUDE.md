...
> **START HERE →** Session 6 (§10.1) is done — **Phase 1 is closed out.** Board/Group/Item/ColumnDefinition now all have create+rename+delete (soft delete throughout); `board.create` auto-creates a starter `Group` ("Tasks") *and* a starter `text` column ("Task name") so a fresh board is immediately usable with no separate "create group"/"create column" UI. A real permission bug found and fixed: `workspace.list` returned every workspace with no role check — a GUEST saw the full workspace sidebar, violating §5; new `lib/permissions/requireOrgRole.ts` fixes it and gates `board.create` (Owner/Admin, §5). Activity feed (`activity.list` + `ActivityFeed`) is the first reader of `ActivityLog`, which every mutation has been writing since Session 2. First Playwright test passes (`tests/e2e/happyPath.spec.ts`: sign in → create board → add items → inline-edit → keyboard-reorder), authenticated by injecting a database-session cookie directly (no OAuth, no test-only backdoor route — `authOptions` uses `session: {strategy: "database"}`, so a `Session` row *is* the credential). **This one test found three real, severe bugs on its first run** — a missing `"use client"` on every column-type file that broke the entire tRPC API route with a 500, a virtualizer never measuring real row heights, and dnd-kit/TanStack Virtual silently fighting over the CSS `transform` property so every row rendered stacked at the same position (see §14) — none of which typecheck/lint/Vitest/curl had caught across five prior sessions. Next action is **Phase 2** per §10 — Kanban, Calendar, shared filter/sort/group/search controls, saved views, `dropdown`/`timeline` columns.
