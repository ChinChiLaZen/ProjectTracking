...
> **START HERE →** Session 16 is done — **file attachments, the last unbuilt piece of Phase 3's named scope.** New `Attachment` model (`{organizationId, boardId, itemId, uploaderId, fileName, mimeType, fileSize, storageKey, deletedAt?}`), Item-scoped like `Update` (no per-comment attachments). **Storage is a swappable interface** (`lib/storage/`'s `StorageAdapter`, `upload`/`download` only) — `s3Adapter.ts` (`@aws-sdk/client-s3`) and `localDiskAdapter.ts` (writes under `LOCAL_UPLOAD_DIR`, default `./.uploads`, gitignored), picked automatically by whether `S3_BUCKET` is set, so local dev needs zero new required config. **Creation can't go through tRPC** (no multipart transport) — `POST /api/attachments/upload` is the **first non-tRPC authenticated route** in this codebase, using Web-standard `request.formData()`/`File` (no multer/formidable) and a new `server/http/routeAuth.ts`'s `authorizeBoardRequest()` to get the identical `requireBoardAccess` guarantee every tRPC procedure has, outside tRPC. `server/trpc/context.ts`'s session→org resolution was extracted into `resolveSessionContext()` so this route and `createContext` share one implementation, not two copies. **§12's "content-type sniffing, not just extension" is real, not aspirational**: `lib/storage/validateUpload.ts` uses `file-type`'s magic-byte detection against a deliberately narrow allowlist (images + PDF + text/CSV) — the declared filename is never trusted for acceptance, only used as a fallback for the two signature-less formats. Both upload and download proxy through the Next.js server rather than using presigned S3 URLs — a flagged MVP cut (`@aws-sdk/s3-request-presigner` isn't installed), fine until file size/volume actually demands it. All of Phase 3's named scope (comments, @mentions, attachments, notifications, activity feed) is now built — **but Phase 3 isn't fully "done"**: Session 15's flagged gap stands — `deliverMentionEmails()` is still unscheduled, so email notifications don't actually go out continuously in production yet. Next action: Phase 4 (the BullMQ automation worker) is the natural next phase — no session plan exists for it yet in §10.1 (§2.1's two pre-Session-1 questions were already answered: always-on worker service, not serverless-cron), and standing up that worker is also the moment to finally schedule `deliverMentionEmails()`, closing Session 15's gap as a side effect rather than a separate task.
...
- **In-board search** filters the current `useBoardData` query against `valueText`/`valueRefIds` shadow columns — same path as filters, just a free-text predicate. *(Still unbuilt as a dedicated UI — the filter builder already covers per-column "contains" today.)*
- **Global search** (across boards in a workspace) is a separate, explicitly-scoped query: Postgres full-text (`tsvector` on `valueText` + item name) is the default per §13; it must still run `requireBoardAccess` per matched board so results never leak items from boards the user can't open. **Built in Session 12** — `server/services/search.ts`'s `searchWorkspace`, `search.global` procedure, a per-workspace search box on the home page. `tsvector` is computed inline at query time (no generated/indexed column yet — flagged perf limitation, see §14).
...
- `2026-07-24` — Session 12 shipped: global search (§6.1), the last named Phase 2 item. `server/services/search.ts`'s `searchWorkspace` is the codebase's first `$queryRaw` — Postgres full-text search has no first-class Prisma schema representation, so `to_tsvector('english', ...)` is computed inline at query time against `Item.name` and `ColumnValue.valueText` rather than via a generated/stored column with a GIN index. This is a real, explicitly flagged perf limitation (sequential scan, not indexed) — not solved this session, matching the "measure before optimizing" discipline Session 4 established for the 10k-item board. Two match sources (item name, column `valueText`) are combined with `UNION ALL` (not `UNION` — the plan's original reasoning was `UNION` since `UNION ALL` wasn't yet decided, but since app code already dedupes by max rank per `itemId` afterward — an item matching both sources would otherwise appear twice with different rank values, which `UNION`'s byte-for-byte row dedup can't catch anyway — `UNION ALL` avoids Postgres paying for a `DISTINCT` pass that wouldn't achieve the real goal regardless).
- `2026-07-24` — Important, easy-to-miss architectural note: **the tenant-scoping Prisma Client Extension (`server/db/client.ts`) does not intercept `$queryRaw`** — it only hooks the standard CRUD methods (`findMany`, `create`, etc.). Every other service function gets tenant-isolation defense-in-depth "for free" from that extension on top of its own explicit `organizationId` predicate; `searchWorkspace` does not — its explicit `organizationId`/`workspaceId` WHERE clauses in the raw SQL are the *only* tenant boundary. Worth remembering for any future raw-SQL query: the extension's safety net doesn't apply, the explicit predicate has to be right on its own.
- `2026-07-24` — Board access for search is checked once per **distinct matched board**, not once per result row, via the existing `requireBoardAccess({userId, organizationId}, boardId, "GUEST")` called directly from the service layer (no router dependency — same pattern `tests/crud.integration.test.ts` already established for calling it outside a procedure). A board that fails the check has all of its matches silently dropped, never surfaced as an error — this is what makes a GUEST's search results naturally scoped to only the boards they're an explicit member of, with no separate "is this caller a guest" branch anywhere in the query or service; verified directly in `tests/globalSearch.integration.test.ts` (a GUEST's search for a term matching items on both their board and a board they're not a member of returns only the former).
- `2026-07-24` — Session 13 shipped: `Update` (§4.1's comment-thread model), Phase 3's first slice. Shipped as **plain text against §12's "rich text" framing, flagged explicitly** rather than either silently ignoring the spec or pulling in a rich-text editor + HTML-sanitization library for a feature with no rich-text renderer yet to justify it. `body` is a validated plain string (`z.string().trim().min(1).max(5000)`), rendered with `white-space: pre-wrap` — the exact pattern `long_text`'s `Cell` already uses. Because nothing ever calls `dangerouslySetInnerHTML` on it, React's default escaping is a real (if narrower-than-spec) render-side XSS defense for free; the "rich text" §12 actually describes — and its sanitization requirement — becomes necessary once @mentions need to render inline chips, not before.
- `2026-07-24` — `createUpdate`/`deleteUpdate` write `ActivityLog`+`OutboxEvent` in the same transaction as the data change, matching `item.create`/`setColumnValue` — **not** the `View`-mutations-are-board-configuration exemption Session 7 established. A comment is real board *data* activity by any reasonable reading, so it gets the full transactional-outbox treatment; the existing activity feed (Session 6) becomes a second reader with zero new code. `deleteUpdate`'s permission (creator-or-ADMIN) is a direct, deliberate reuse of `deleteView`'s shape (`server/services/views.ts`) — same reasoning: an author can always remove their own content, ADMIN can moderate anyone's.
- `2026-07-24` — UI is a lightweight `position: fixed` overlay (`components/board/UpdatesPanel.tsx`), not an in-row expansion inside `BoardTable`'s virtualized rows — deliberately, to avoid any risk of re-triggering Session 6's dnd-kit/TanStack-Virtual `transform` collision (two layout systems silently fighting over one row's positioning). Opened via a new "💬" button added to each item row; `gridTemplateColumns` (`BoardTable.tsx`) gained one more fixed-width column for it, which meant updating three places in lockstep to keep the grid aligned: the column-header row, `GroupHeaderRow`'s spanning name cell (needed a matching empty placeholder cell, not a span-count change), and `ItemRow` itself. The panel shows a "Delete" button only for the *viewer's own* comments — there's still no "list board members with roles" query (same gap Session 5 flagged for the person-picker), so an ADMIN's moderation override has no client-side trigger yet even though the server already supports it.
- `2026-07-24` — No edit capability and no `version` field on `Update` this session — create+list+delete is enough to prove the mechanism, the same bar Session 7 used for `View`'s first slice (and for the same reason: there's nothing yet that would make editing meaningfully different from delete-and-repost). §4.2's optimistic-concurrency mandate only names `Item`/`ColumnValue`; `Update` follows the `Group`/`Board`/`ColumnDefinition`/`View` last-write-wins precedent instead. Flagged, not forgotten — revisit if a future session's UX genuinely needs in-place editing.
- `2026-07-24` — Session 14 shipped: @mentions, Phase 3's second slice. Mention tokens are `@[Display Name](userId)` embedded directly in `Update.body` — chosen over fuzzy-matching plain `@name` text (ambiguous once two members share a name, or a name changes) and over a real structured rich-text format (out of scope; a narrow syntactic exception to Session 13's plain-text choice, not a reopening of it). `lib/mentions/mentions.ts` (pure, unit-tested) holds the regex extraction (`extractMentionTokens`/`extractMentionedUserIds`), the render-side splitter (`splitBodyIntoSegments`), and the authoring-side helpers (`detectActiveMentionQuery`, `insertMentionToken`) — all framework-free, matching the established "pure logic tested, UI untested" split (`lib/views/compileQuery.ts`, `lib/views/deriveDraftConfig.ts`, etc.).
- `2026-07-24` — New `Update.mentionedUserIds String[]` column (GIN-indexed: `@@index([mentionedUserIds], type: Gin)`), migration `20260724150928_add_update_mentioned_users` — a **derived, validated** projection of `body`, explicitly mirroring `ColumnValue.valueRefIds` (§4.2): `body` stays canonical/rendered, this is the indexable shadow. `createUpdate` (`server/services/updates.ts`) computes it by regex-extracting every token from `body` and intersecting the ids against `listBoardMembers`'s real member set *before* opening the write transaction (a read, not a write — doesn't need `tx`) — a client can't fabricate a mention naming an arbitrary/unauthorized user id and have it "count." Nothing reads `mentionedUserIds` back yet (no notification consumer exists this session) — same "build the capability now, let a later session consume it" precedent as `OutboxEvent` (Phase 1) and `rebalanceRanks` (Session 3), both built before their consumers existed.
- `2026-07-24` — New `server/services/boardMembers.ts`'s `listBoardMembers` + `board.listMembers` procedure (`GUEST` — board.read baseline) is genuinely new, reusable infra: no "list a board's members" query existed anywhere before this session (confirmed via search — `PersonEditor`'s own Session-5 comment already flagged the gap). It replicates `requireBoardAccess`'s single-user precedence (`lib/permissions/requireBoardAccess.ts`) across every org member at once: explicit `BoardMembership` role wins; otherwise org `Membership` role applies unless it's `GUEST` (no default board access). **Real tenant-isolation bug caught while writing the cross-org test, not shipped**: `BoardMembership` carries no `organizationId` column and isn't in the tenant-scoping extension's model set (`server/db/client.ts`), so a naive `boardMembership.findMany({where: {boardId}})` has no tenant boundary at all if a caller passes a mismatched `organizationId`. Fixed by validating `board.findFirst({id: boardId, organizationId})` first (same defense-in-depth shape `requireBoardAccess` itself uses) and returning `[]` on a miss, matching every other list query's "cross-org returns empty, not a leak" precedent. Also unblocks (but does **not** fix — kept out of this session's scope) `person`'s raw-id display and placeholder comma-separated-ids editor, flagged since Session 5.
- `2026-07-24` — Chip **rendering** is deliberately independent of `mentionedUserIds` — it re-parses `body` directly (the client already has the raw token text) and resolves each mention's *current* name live against the already-fetched `board.listMembers` result, falling back to the name embedded in the token at authoring time if that user's no longer a board member. This means a renamed user's old mentions update automatically, and rendering never needs its own extra round trip. `MentionAutocomplete.tsx` (presentational only; `UpdatesPanel.tsx` owns all interaction state — query detection, filtering, keyboard nav — matching how that file already owned its own state before this session) is positioned near the textarea with a simple fixed offset, **not real caret-position tracking** — flagged MVP simplification, same discipline as Kanban's append-only drop and Calendar's read-only view.
- `2026-07-24` — Two real bugs caught by unit tests before they shipped, not theoretical: (1) `insertMentionToken` naively appending a token (which already ends in a trailing space) produced a double space when the mention was inserted mid-sentence, right before existing whitespace (e.g. "hey @ali there" → "hey @[Alice Smith](user-1)  there"). Fixed by skipping one character of existing whitespace immediately after the replaced span. (2) An early test asserted a wrong token-length offset by hand-counting characters incorrectly — caught immediately by the test itself failing, not a logic bug, but a reminder that hand-computed offsets in fixtures are exactly the kind of "known fact" Session 10's calendar-grid tests already learned to verify by computation rather than by hand.
- `2026-07-24` — Session 15 shipped: notifications, Phase 3's third slice. New `Notification` model (`recipientId, boardId, itemId, updateId, type, readAt?, emailSentAt?`), migration `20260724171049_add_notifications`, `@@unique([recipientId, updateId])` as the literal "deduped" mechanism the done-when names. **Split the two halves of delivery on a genuine architectural line, not an arbitrary one**: in-app `Notification` rows are written in `createUpdate`'s own transaction (`server/services/updates.ts`) — first-party application data, same reasoning `ActivityLog` already uses, no queue needed, self-mentions excluded (`recipientIds = mentionedUserIds.filter(id => id !== authorId)`). Email crosses a real external boundary (SMTP), which is exactly what §7.1's outbox/relay pattern exists for — `server/services/notificationRelay.ts`'s `deliverMentionEmails()` is the first-ever consumer of `OutboxEvent`, which needed its `item.update_created` payload extended to carry `mentionedUserIds` (it only had `{updateId, body}` before) so the relay is self-contained.
- `2026-07-24` — **Per explicit user answer, `deliverMentionEmails()` is deliberately left unscheduled** — a plain, fully-tested, callable function, same precedent already set for `rebalanceRanks` (Session 3: "build the capability now, wire the scheduler in Phase 4"). This is a real, named gap, not a silently-accepted one: Phase 3's done-when ("a mention reliably produces one notification and one email") isn't *fully* true in production until something calls this on a schedule — flagged explicitly in the START HERE line, not just here. `prisma/seed.ts` calls it once at the end so `pnpm db:seed` proves the whole path end-to-end (a real captured email, visible via the new `maildev` `docker-compose.yml` service at `localhost:1080`) — a one-time proof, not continuous delivery. `lib/mailer.ts`'s `sendEmail` reuses the existing `EMAIL_SERVER`/`EMAIL_FROM` env vars NextAuth's `EmailProvider` already uses for magic links — same SMTP transport, a second independent call site, not a shared code path with NextAuth's own internal usage.
- `2026-07-24` — `notification.list`/`unreadCount`/`markRead`/`markAllRead` are session-scoped (`recipientId === ctx.userId` + `organizationId` for tenant safety), not board-scoped — the first router in this codebase with no `requireBoardAccess` gate anywhere. A notification is personal data, closer to `workspace.list`'s org-level scoping (Session 6) than any board-level procedure. `markRead` does a plain ownership check (`notification.recipientId !== callerId` → `FORBIDDEN`) instead of a role check — no ADMIN moderation override makes sense for someone else's personal notification, consistent with how ADMIN already can't see someone else's `PERSONAL` view either (Session 7).
- `2026-07-24` — Two real bugs caught while writing this session's own tests, not shipped: (1) `markRead` and both `update` calls inside `notificationRelay.ts` initially omitted `organizationId` from their `where` clause — the tenant-scoping extension (`server/db/client.ts`) caught this immediately and loudly (`"Tenant scoping violation: ...update missing/mismatched organizationId"`), exactly the fail-loud behavior that extension exists for. Fixed by adding `organizationId` alongside `id` in each `where`. (2) A genuine race condition, not just test flakiness: `deliverMentionEmails()` is deliberately global (no org filter — a real relay must see every org's unpublished events at once, same reasoning `searchWorkspace`'s raw SQL is intentionally cross-tenant per Session 12's decision log) — running the full test suite in parallel, another test file's `afterAll` cleanup occasionally hard-deleted an `OutboxEvent`/`Notification` row between this relay's initial read and its later `update`, throwing Prisma's "record not found" (P2025) and failing the whole batch. Fixed by switching both terminal writes (`Notification.emailSentAt`, `OutboxEvent.publishedAt`) from `update` to `updateMany` — it matches zero rows instead of throwing when the target has vanished, which is *also* correct production behavior for an at-least-once relay (a row can legitimately disappear between read and write if, say, a future second relay instance processed it first), not merely a test workaround.
- `2026-07-24` — `deliverMentionEmails`'s own tests key their assertions off a unique marker string in each test's comment body (e.g. `"relay test one"`) rather than the function's aggregate `processed`/`sent` counts, and pass a generous explicit `limit` (1000) — because the function scans globally, aggregate counts and default limits are exactly the kind of assertion that would be flaky under parallel test-file execution (another file's fixtures could inflate or crowd out counts). Worth remembering as the general pattern for testing any future global/cross-tenant relay-style function: assert on your own fixture's specific, uniquely-identifiable state, never on the function's aggregate return values.
- `2026-07-25` — Session 16 shipped: file attachments, closing out Phase 3's named scope. New `Attachment` model (migration `20260724181412_add_attachments`), Item-scoped like `Update` (no per-comment attachments this session — flagged as a possible future extension, not built). `lib/storage/`'s `StorageAdapter` interface is deliberately minimal (`upload`/`download` only, moving bytes not metadata) — `Attachment.mimeType` (the DB row) is the canonical content type, never whatever a storage backend reports back, matching how this codebase already prefers canonical DB values over embedded/backend ones elsewhere (status columns store `optionId`, never the label). `s3Adapter.ts` / `localDiskAdapter.ts` are picked by `getStorageAdapter()` (`lib/storage/index.ts`) automatically based on whether `S3_BUCKET` is set — zero new required config for local dev, matching §2.1's philosophy. New env var: `LOCAL_UPLOAD_DIR` (default `./.uploads`, gitignored).
- `2026-07-25` — **First non-tRPC authenticated route in this codebase**: `POST /api/attachments/upload` and `GET /api/attachments/[attachmentId]`, needed because tRPC's JSON-RPC transport has no multipart support — Next.js Route Handlers' Web-standard `request.formData()`/`File` API handles the upload natively, no `multer`/`formidable`/`busboy` dependency needed. `server/trpc/context.ts`'s session→org resolution logic was extracted into an exported `resolveSessionContext()` (both it and `createContext` now call it) so there's one implementation, not two independently-drifting copies. New `server/http/routeAuth.ts`'s `authorizeBoardRequest(boardId, minRole)` gives both routes the exact same `requireBoardAccess` guarantee every tRPC procedure already has, mapping a thrown `TRPCError` to the equivalent HTTP status (404/403/401) — extracted once during this session (not duplicated across the two routes that both needed it) rather than left as a hypothetical "generalize later." The download route loads the `Attachment` row first (tenant-scoped by the caller's own `organizationId` — missing/wrong-org is indistinguishable from not existing, the same anti-probing shape `requireBoardAccess` itself uses) to learn which board's permission applies, rather than taking a `boardId` as a separate, spoofable query param.
- `2026-07-25` — §12's "Validate file uploads by content-type sniffing, not just extension" is real, not aspirational, from the start — `lib/storage/validateUpload.ts` uses `file-type`'s magic-byte detection; the declared filename is *never* trusted for acceptance, only used as a controlled fallback for the two formats with no byte signature to sniff (plain text/CSV). Verified this actually matters, not just in theory: a unit test posts a file named `photo.jpg` whose real bytes are a shell script — sniffing (correctly) rejects it regardless of the trusted-looking extension. The MIME allowlist is deliberately narrow this session (images + PDF + text/CSV) — office-document formats (docx/xlsx) are technically ZIP containers, and `file-type`'s exact detection behavior for them wasn't verified against a real fixture, so they're left out rather than assumed to work. Both upload and download proxy through the Next.js server rather than using presigned S3 URLs for direct browser↔S3 transfer (`@aws-sdk/s3-request-presigner` isn't installed) — a flagged MVP cut, same "ship the synchronous version, flag the optimization" discipline as Kanban's simple positioning and Calendar's simple grid; revisit once file size/volume actually makes the server-proxy path a bottleneck. §12's "isolate the S3 bucket from direct public write" is satisfied structurally by this same design — the client never talks to S3 directly, only the server does, using IAM credentials, so there's no public-write bucket policy to get wrong in the first place.
- `2026-07-25` — All of Phase 3's named scope is now built (comments, @mentions, attachments, notifications, activity feed) — but Phase 3 is still not fully "done" per its own stated gate, because Session 15's flagged gap is untouched by this session: `deliverMentionEmails()` remains unscheduled. Recorded here so it doesn't get lost now that this phase's checklist otherwise reads as complete — closing it is naturally Phase 4's job (the BullMQ worker session), not a separate cleanup task.
