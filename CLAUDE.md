...
> **START HERE →** Session 7 is done — **Phase 2's first slice: saved views.** §10.1 has no session breakdown past Session 6 ("Only then move to Phase 2"), so this session's scope was chosen live: Phase 2's own done-when names it directly ("a saved view URL is shareable"). New `View` model (`{id, organizationId, boardId, name, visibility: SHARED|PERSONAL, creatorId, config: Json, createdAt, deletedAt}`, no `rank` — §4.2's fractional-rank list doesn't include View, `createdAt` ordering is enough); `config` validates against the existing `lib/views/viewConfig.ts` schema from Session 4. Permission split reused capability names that were already sitting in `lib/permissions/matrix.ts` unused: creating a `SHARED` view needs `board.editStructure` (ADMIN), a `PERSONAL` view needs only `view.createPersonal` (GUEST) — `view.create`'s router computes `minRole` dynamically from `input.visibility` before calling `requireBoardAccess`. `getView`/`listViews` hide a `PERSONAL` view from everyone except its creator — **including ADMIN** — and 404 rather than 403 (same anti-probing shape as `requireBoardAccess`); `deleteView` is the only place ADMIN gets an override (board-config cleanup, not a privacy grant). The `[[...view]]` route segment (reserved since Session 2, ignored until now) is read as a `View.id`; `BoardTable` fetches it via `trpc.view.get` and threads the resulting `viewConfig` into every group's `item.list` call — sharing a view's URL literally shares that exact `viewConfig`. Hit the same "Json field through react-query generics is excessively deep" TS error from Sessions 2/4 (see §14) — fixed the same way, trimming `config`'s Prisma `Json` type to `unknown` at the router boundary. **No filter/sort-builder UI yet** — the only `viewConfig` a user can currently save is the default (table, no filters, no sort); saving/sharing still exercises the real persistence+URL mechanism end-to-end, same reasoning Session 4 used to ship pagination before any filter UI existed to drive it. `view.update`/rename also deferred — create+list+get+delete was enough to prove the mechanism. Next action: pick the next Phase 2 slice (Kanban, Calendar, or the filter/sort-builder UI saved views are now ready to persist) — no session plan exists for it yet in §10.1, write one before starting.
