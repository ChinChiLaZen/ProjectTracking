...
> **START HERE →** Session 14 is done — **@mentions, Phase 3's second slice.** Mentions are `@[Display Name](userId)` tokens embedded directly in `Update.body` (no schema change to `body` itself, no rich-text editor — a narrow syntactic exception to Session 13's plain-text choice, not a reopening of it). A new `Update.mentionedUserIds String[]` column (GIN-indexed, mirroring `ColumnValue.valueRefIds` per §4.2) is a **derived, validated** projection: `createUpdate` regex-extracts every token from `body` and intersects the ids against the board's real member set (a client can't fabricate a mention naming an unauthorized user id and have it "count"), but nothing reads it back yet — same "build the capability now" precedent as `OutboxEvent` (Phase 1) and `rebalanceRanks` (Session 3), both built before their consumers existed. Chip **rendering** is purely syntactic and independent of `mentionedUserIds` — it re-parses `body` client-side (`lib/mentions/mentions.ts`), resolving each mention's *current* display name live against a new `board.listMembers` query, falling back to the name embedded in the token if that user's no longer on the board. `board.listMembers` (`server/services/boardMembers.ts`) is new, reusable infra replicating `requireBoardAccess`'s single-user precedence (`BoardMembership` override, else org `Membership` unless `GUEST`) across every board member at once — it also unblocks (but does **not** fix, kept out of scope) the `person` column's long-flagged raw-id-display/picker gap (Session 5). Authoring UX: typing `@word` in the comment textarea shows a filtered autocomplete dropdown (`MentionAutocomplete.tsx`) positioned with a simple fixed offset, not real caret tracking — flagged MVP simplification. Notification delivery (in-app/email) is explicitly **not** this session — Phase 3's own done-when is the next slice this one exists to unblock; no session plan for it exists yet in §10.1, write one before starting.
