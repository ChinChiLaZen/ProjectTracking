...
> **START HERE →** Session 12 is done — **global search (§6.1), the last named Phase 2 item.** `server/services/search.ts`'s `searchWorkspace` is the first `$queryRaw` in the codebase — Postgres full-text search (`to_tsvector`/`plainto_tsquery`/`ts_rank`) has no first-class Prisma schema representation, so it's computed inline at query time rather than via a generated/stored tsvector column with a GIN index. That's a **sequential scan, not an indexed lookup** — a known, explicitly flagged perf limitation, not assumed away; revisit if a real workspace's data volume makes it slow (same "measure before optimizing" discipline Session 4 used for the 10k-item board). Important: **the tenant-scoping Prisma Client Extension does not intercept `$queryRaw`** — it only hooks the standard CRUD methods — so this query's explicit `organizationId`/`workspaceId` WHERE clauses are the *only* tenant boundary, not a defense-in-depth backstop on top of the extension like every other service function gets for free. Matches across item name and column `valueText` are combined via `UNION ALL` and deduped by max rank in application code (an item matching both would otherwise appear twice with different rank values — `UNION`'s row-level dedup wouldn't catch that since the rows aren't byte-identical). Board access is checked once per **distinct matched board**, not per row, via the existing `requireBoardAccess({userId, organizationId}, boardId, "GUEST")` — a board that fails silently drops all its matches rather than erroring, which is what makes a GUEST's search naturally scoped to only the boards they're an explicit member of, with no separate guest-specific branch anywhere in the query or service. Next action: Phase 2 is now fully closed (Kanban, Calendar, filter/sort-builder, saved views, `dropdown`, `timeline`, global search) — remaining Phase 2 line items are the in-board "search across all columns" convenience UI and CSV import/export (§6.1's other two bullets, still unbuilt) or `link`/`files`/`connect_board`/`formula`/`rollup`/`rating` (still "Later" per §4.3), or consider Phase 2 closed and move to Phase 3 (Collaboration) — no session plan exists for any of these yet in §10.1, write one before starting.
