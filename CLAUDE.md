...
> **START HERE →** Session 9 is done — **Phase 2's third slice: Kanban.** §10.1 has no session breakdown past Session 6; per your answer this session builds "groups by any `status`/`dropdown`/`person` column; drag = one `setColumnValue` + one `rank` update in a single transaction" (§6), reusing `groupKeys()` (built Session 2/5) and `viewConfig.type`/`groupBy` (reserved since Session 4, unused until now). New `moveKanbanItem` (`server/services/items.ts`) is the one genuinely new mutation — a single `$transaction` combining an item rank/version update with `writeColumnValueInTx` (extracted from `setColumnValue`, `server/services/columnValues.ts`, so both callers share identical validate/shadow-project/version-check logic), writing both `item.moved` and `item.column_changed` `ActivityLog`/`OutboxEvent` rows since §7 treats those as separate automation trigger types. **Deliberately scoped down, three ways, all flagged rather than silently assumed**: (1) "group by" is restricted to `status`/`person` only (`lib/views/kanbanGroupKeyValue.ts`'s `isKanbanGroupable`) — every column type technically has `groupKeys()`, but only these two have a sane *inverse* (bucket → value), and building a generic one would mean a new §4.3 registry hook for a need only one feature has; (2) no manual reordering within a Kanban bucket — dropping on a different bucket always appends to its end, dropping on the same bucket is a no-op; precise per-card drop targets are real added DnD complexity for a non-core interaction; (3) Kanban cards are read-only (name only), no inline editing. `person`'s inverse mapping is source-*and*-target aware (`valueForGroupKey`), not target-only — dragging a card from Alice's column to Bob's means "unassign Alice, assign Bob," matching Decision 3's multi-bucket framing, not "wipe every assignee." `bucketItemsByGroupKeys` (`lib/views/`) special-cases `status` to enumerate every configured option — even ones with zero items — in configured order, closing the sort-order gap Session 5 flagged back when no Kanban view existed to make it matter; `person` still only buckets observed values (no board-membership list exists yet, same flagged gap as the person-picker). Found and fixed a real bug in the process: `FilterSortBuilder`'s `onChange` was reporting a full `ViewConfig` that silently reset `type`/`groupBy` back to schema defaults on every filter edit — narrowed to report only `{filters, sort}`, which `BoardTable` now merges into `draftConfig` instead of replacing it. Seed gained a third view ("By status (Kanban)"). **A live browser test of the drag surfaced a real, pre-existing production bug**, not a Session 9 regression: `item.setColumnValue` (inline cell editing, unchanged since Session 2) and the new `item.moveKanban` both threw `TypeError: Cannot read properties of undefined (reading 'parse')` — confirmed via a direct Node script (service logic correct), a full `.next` cache wipe + process kill + clean `pnpm dev` restart (still failed), a production `pnpm build && pnpm start` (still failed), and finally an isolated `git worktree` built from Session 8's last commit `e10bf5f` (failed identically there too — meaning this has been silently broken since at least Session 8, because no session since Session 6 exercised a live browser mutation). Root cause: every `lib/columnTypes/<type>.tsx` file was `"use client"` (needed for its `Cell`/`Editor` components) and exported ONE object mixing those components with plain server-usable logic (`valueSchema`, `toShadow`, etc.) — Next's RSC bundler wraps *every* export of a `"use client"` file as a client reference when reached from server code, including non-component exports, so `columnType.valueSchema` resolved to `undefined` server-side. Fixed by splitting all 7 column types into `<type>.ts` (logic, no `"use client"`, assembles the same `<type>Column` export) + `<type>.client.tsx` (`"use client"`, just `Cell`/`Editor`) — `registry.ts` and all 7 test files needed **zero changes** (same export names/shapes). Verified: 221 tests pass unmodified, a rebuilt server returns 200 (not 500) for both procedures, and a real keyboard-driven Kanban drag in the browser persisted correctly across a page reload. Next action: Calendar is the last named Phase 2 slice (`date`-driven, `timeline` column type doesn't exist yet) — no session plan exists for it yet in §10.1, write one before starting.
