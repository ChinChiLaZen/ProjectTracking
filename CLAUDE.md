...
> **START HERE →** Session 5 (§10.1) is done: all Phase 1 column types now exist — `status`, `person`, `date`, `number`, `checkbox`, `long_text` alongside `text`/Session 2's original. Every type was in fact near-mechanical against the registry/shadow/filter machinery from Sessions 2–4, confirming that machinery holds — no registry-interface changes were needed. Three real gaps found and deliberately deferred, not silently papered over (see §14): `status` sorts alphabetically by `optionId` rather than configured option order; `person`'s `Editor` is a placeholder comma-separated-ids text input, no real people-picker exists (needs a `board.listMembers`-shaped query that doesn't exist yet); `date` treats its value as UTC, ignoring the board's actual timezone. Next action is **Session 6** in §10.1 — Phase 1 close-out: Board/Group/Item CRUD polish, role enforcement across every procedure, activity feed, Playwright happy path. Gate: Phase 1 *done-when* — you can run a real project on it.
