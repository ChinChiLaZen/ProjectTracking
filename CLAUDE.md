...
> **START HERE →** Session 3 (§10.1) is done: `lib/ordering/rank.ts` extended with a general `rankBetween` (+ `isValidRank`, added after a real crash was found mid-session — see §14), `lib/ordering/rebalance.ts`'s `rebalanceRanks` (callable, not yet scheduled — no BullMQ), `moveItem`/`moveGroup` services with the single-row-update gate asserted in `tests/reorder.integration.test.ts`, and dnd-kit wired into `BoardTable` (pointer + keyboard sensors, nested sortable contexts for groups and same-group items). Next action is **Session 4** in §10.1 — the 10k-item board, virtualization, cursor pagination, and the filter→SQL compiler against shadow columns. Do this before adding more column types.
