...
> **START HERE →** Session 13 is done — **item updates/comments, Phase 3's first slice.** Per your answers, this moves off Phase 2 (fully closed as of Session 12) without chasing its remaining minor bullets (in-board search UI, CSV import/export) and starts Collaboration with the foundational piece the rest of the phase needs: `Update` (§4.1's comment-thread model, unbuilt until now). **Deliberately plain text, not the "rich text" §12 describes** — no rich-text editor or HTML-sanitization library exists in this codebase; content is a validated (`.trim().min(1).max(5000)`) plain string rendered with `white-space: pre-wrap` (matching `long_text`'s Cell), which gets React's default escaping for free since nothing ever uses `dangerouslySetInnerHTML`. That satisfies the *spirit* of §12's render-side defense for plain text, but not what "sanitize rich text" actually means — flagged explicitly, revisit once @mentions (the next Phase 3 slice) need inline chip rendering, which is the point rich text actually becomes necessary. `createUpdate`/`deleteUpdate` write `ActivityLog`+`OutboxEvent` like `item.create`/`setColumnValue` do — an Update is real board *data* activity, the opposite of `View`'s "board configuration" exemption from Session 7 — so the existing activity feed becomes a free second reader with zero extra work. Deletion reuses `deleteView`'s exact creator-or-ADMIN shape. UI is a lightweight `position: fixed` overlay (`UpdatesPanel`, opened via a "💬" button added to each item row's grid — `gridTemplateColumns` gained one more fixed column), deliberately **not** an in-row expansion, to avoid any risk of repeating Session 6's dnd-kit/TanStack-Virtual `transform` collision (two layout systems fighting over one row). No edit capability and no `expectedVersion` this session — create+list+delete is enough to prove the mechanism, the same bar Session 7 used for `View`. Next action: @mentions (parsing update bodies, resolving `@name` to a user, rendering as a chip) is the next Phase 3 slice — it's what makes notifications (in-app + email, Phase 3's own done-when) have something to trigger on; no session plan exists for it yet in §10.1, write one before starting.
