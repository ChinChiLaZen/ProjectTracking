...
> **START HERE →** Session 10 is done — **Phase 2's fourth slice: Calendar.** Scoped to `date`-only per Session 9's own note (`timeline` doesn't exist as a column type yet). New `listItemsInDateRange` (`server/services/items.ts`) is the first **boardwide** item query — every prior list (`listItemsInGroup`, Kanban's per-Group swimlanes) is scoped to one real board Group, but a calendar day cell needs items from every group, so this is a genuinely new query shape, not a variant of an existing one. Reuses `compileViewConfig` for the *other* filters and ANDs in its own always-present `values: {some: {columnId: dateColumnId, valueDate: {gte, lte}}}` condition; no cursor pagination (a rendered month grid is inherently bounded) but a flat `limit` (default 500) keeps §8's "never load the whole board" literally true against a pathological all-items-one-day case. `lib/views/calendarGrid.ts`'s `getMonthGrid` is the pure date-math the whole feature sits on (Sunday-start weeks, 4–6 rows depending on alignment, not padded to a fixed 6) — **the query range is the rendered grid, not the calendar month**, so an item due on a leading/trailing padding day (visually in the grid, technically in the adjacent month) still shows up. The "Date column" picker (`components/board/BoardTable.tsx`) filters columns by `getColumnType(registry, c.key).shadowField === "valueDate"` rather than hardcoding column keys the way Kanban's `isKanbanGroupable` had to — a future `timeline` column type (same shadow field) slots into the picker with zero Calendar-side changes, an improvement over Kanban's approach worth carrying forward whenever a new column-type-restricted picker is needed. **Deliberately scoped down, flagged not discovered later**: read-only (no drag-to-reschedule — Table view's inline date editor already covers rescheduling); no "unscheduled items" list (an item with no value for the chosen column just doesn't appear); `currentMonth` is local component state, not part of the persisted `ViewConfig` — a saved Calendar view remembers *which date column*, not *which month was open*. Inherits the existing UTC-midnight simplification from `date.ts` (flagged since Session 5) unchanged. Next action: Phase 2's named slices (Kanban, Calendar, filter/sort-builder, saved views) are all done — pick the next Phase 2 item (`dropdown`/`timeline` column types, or the shared search UI beyond what Kanban/Calendar already built) or consider Phase 2 closed and move toward Phase 3 (Collaboration) — no session plan exists for either yet in §10.1, write one before starting.
